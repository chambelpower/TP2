...
class LZ78Compressor:
    ...
    def compress(self, inputPath, outputPath):
        ...
        # data1 = bytes(rle.rle(data), 'utf-8')#com rle
        #data1 = bytes(bw.bwt1(str(data)), 'utf-8')  # com bwt

        # com bwt e rle
        # data1 = bw.bwt1(str(data))
        # data1 = bytes(rle.rle(data1), 'utf-8')
        #inp = list(data1)
...
def lz78_compress(inputPath, outputPath):
    lz78_comp = LZ78Compressor()
    lz78_comp.compress(inputPath, outputPath)


"""def main():
    filenames = ['bible.txt', 'finance.csv', 'jquery-3.6.0.js', 'random.txt']
    inputPath = filenames[3]
    outputPath = 'bible2.txt'
    outputPath2 = 'biblie3.txt'
    lz78_c = LZ78Compressor()
    lz78_d = LZ78Decompressor(outputPath)

    compressionStartTime = time.time()
    lz78_c.compress(inputPath, outputPath)
    compressionElapsedTime = time.time() - compressionStartTime

    decompressionStartTime = time.time()
    lz78_d.decompress(outputPath, outputPath2)
