...
def byteWriter(bitStr, outputFile):
    ...
    while len(bitStream) > 8:  # write byte(s) if there are more then 8 bits
        ...
        outputFile.write(chr(int(byteStr, 2)).encode())
...
def sfc(inputFile, outputFile, mode):
    global byteArr
    global bitPosition
    global tupleList
    global bitStream
    ...
    if mode == 'e':  # FILE ENCODING
        ...
        fo.write(chr(len(dic) - 1).encode())  # first write the number of encoding tuples
    ...
    elif mode == 'd':  # FILE DECODING NOT WORKING
        bitPosition = 0
...
outputFile = 'biblia2.txt'

sfc(inputFile, outputFile, mode)"""